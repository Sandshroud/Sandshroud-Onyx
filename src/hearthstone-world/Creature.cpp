...
bool Creature::Load(CreatureSpawn *spawn, uint32 mode, MapInfo *info)
{
	...
	SetEquippedItem(0, proto->Item1);
	SetEquippedItem(1, proto->Item2);
	SetEquippedItem(2, proto->Item3);
...
// PLACE FOR DIRTY FIX BASTARDS
	...
	{
		ItemPrototype* DBCItem = ItemPrototypeStorage.LookupEntry(tmpitemid);
		if(DBCItem)
		{
...
bool Creature::Load(CreatureProto * proto_, uint32 mode, float x, float y, float z, float o)
{
	...
	if(tmpitemid = proto->Item1)
	{
		ItemPrototype* proto = ItemPrototypeStorage.LookupEntry(tmpitemid);
		if(proto)
		{
	...
	if(!b_has_shield && (tmpitemid = proto->Item2))
	{
		...
		if(proto)
		{
			...
			{
				if(proto->InventoryType == INVTYPE_WEAPONOFFHAND
