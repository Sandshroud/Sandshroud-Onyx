...
void QuestMgr::BuildOfferReward(WorldPacket *data, Quest* qst, Object* qst_giver, uint32 menutype, Player* plr)
{
	...
	*data << (qst->qst_next_quest_id ? uint32(1) : uint32(0));	// next quest shit
	*data << uint32(0);
...
{
	if(qst->rewards == NULL || !qst->rewards->reward_title || qst->rewards->reward_title > 63)
		return;
...
void QuestMgr::OnQuestFinished(Player* plr, Quest* qst, Object* qst_giver, uint32 reward_slot)
{
	...
	if(IsQuestRepeatable(qst))
	{
		...
		if(qst->rewards)
		{
			plr->ModUnsigned32Value(PLAYER_CHARACTER_POINTS1, qst->rewards->reward_talents);
