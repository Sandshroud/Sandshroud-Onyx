...
void WorldSession::HandleAutoEquipItemOpcode( WorldPacket & recv_data )
{
	...
	if((Slot == EQUIPMENT_SLOT_MAINHAND || Slot == EQUIPMENT_SLOT_OFFHAND) && !_player->ignoreitemreq_cheat)
	{
		...
		if((error = _player->GetItemInterface()->CanEquipItemInSlot(INVENTORY_SLOT_NOT_SET, Slot, eitem->GetProto(), true)))
		{
	...
	{
		if((error = _player->GetItemInterface()->CanEquipItemInSlot(INVENTORY_SLOT_NOT_SET, Slot, eitem->GetProto(), false)))
		{
			_player->GetItemInterface()->BuildInventoryChangeError(eitem,NULLITEM, error);
			return;
		}
...
void WorldSession::HandleBuyBackOpcode( WorldPacket & recv_data )
{
	...
	if (it)
	{
		...
		{
			data.Initialize(SMSG_BUY_FAILED, 12);
			data << uint64(guid);
			data << uint32(itemid);
		...
		data.Initialize( SMSG_BUY_ITEM, 16 );
		data << uint64(guid);
