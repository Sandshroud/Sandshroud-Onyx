...
//	OUT_DEBUG("Building update block for Player"); // Too heavy for the core.
	...
	case TYPEID_GAMEOBJECT:
		{
			flags = (UPDATEFLAG_LOWGUID | UPDATEFLAG_HIGHGUID | UPDATEFLAG_HAS_POSITION);
...
void Object::_BuildMovementUpdate(ByteBuffer * data, uint8 flags, uint32 moveflags, Player* target )
{
	*data << flags;
	...
	{
		if(uThis && uThis->m_TransporterGUID)
...
void Object::_BuildValuesUpdate(ByteBuffer * data, UpdateMask *updateMask, Player* target)
{
	...
	if(updateMask->GetBit(OBJECT_FIELD_GUID) && target)	   // We're creating.
	{
		...
		else if(IsGameObject())
		{
			...
			DummyDynFlags = m_uint32Values[GAMEOBJECT_DYN_FLAGS];
		...
		if(pVictim->IsPlayer())
		{
			...
			if(!(plrVictim->m_bg && plrVictim->m_bg->IsArena())) // Can't self res in Arena
			{
				...
				if( !self_res_spell && plrVictim->bReincarnation )
				{
					...
					if( m_reincarnSpellInfo != NULL && plrVictim->Cooldown_CanCast( m_reincarnSpellInfo ) )
						if( plrVictim->GetItemInterface()->GetItemCount( 17030 ) )
...
//------------------------------by stats----------------------------------------------------
	if( IsUnit() && !static_damage )
	{
		...
		{
			if(spellInfo->SpellFamilyFlags.IsFitToFamilyMask(it->second.first))
			{
...
//------------------------------critical strike chance--------------------------------------
			...
			else
			{
				...
				{
					SM_FFValue(caster->SM[SMT_CRITICAL][0], &CritChance, spellInfo->SpellFamilyFlags);
					SM_PFValue(caster->SM[SMT_CRITICAL][1], &CritChance, spellInfo->SpellFamilyFlags);
				}
...
//==========================================================================================
			if (critical)
			{
				...
				if( spellInfo->SpellFamilyFlags )
					SM_FIValue( caster->SM[SMT_CRITICAL_DAMAGE][1], &critical_bonus, spellInfo->SpellFamilyFlags );
...
void Object::SendSpellNonMeleeDamageLog( Object* Caster, Unit* Target, uint32 SpellID, uint32 Damage, uint8 School, uint32 AbsorbedDamage, uint32 ResistedDamage, bool PhysicalDamage, uint32 BlockedDamage, bool CriticalHit, bool bToset )
{
	...
	uint32 Hit_flags = (0x00001|0x00004);
	if(CriticalHit)
		Hit_flags |= 0x00002;
