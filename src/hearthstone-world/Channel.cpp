...
{
	Guard mGuard(lock);
	uint32 cid = 0, team = ((seperatechannels && p != NULL) ? p->GetTeam() : 0);
	for(ChannelList::iterator itr = Channels[team].begin(); itr != Channels[team].end(); itr++)
	{
		if(!stricmp(name, (*itr)->m_name.c_str()))
		{
			return (*itr);
		}
	}
	...
	cid = ++m_idHigh;
	Channel *chn = new Channel(name, team, type_id, cid);
	Channels[team].push_back(chn);
...
{
	Guard mGuard(lock);
	uint32 team = ((seperatechannels && p != NULL) ? p->GetTeam() : 0);
	for(ChannelList::iterator itr = Channels[team].begin(); itr != Channels[team].end(); itr++)
	{
		if(!stricmp(name, (*itr)->m_name.c_str()))
		{
...
void ChannelMgr::RemoveChannel(Channel * chn)
{
	...
	uint32 team = (seperatechannels ? chn->m_team : 0);
	for(ChannelList::iterator itr = Channels[team].begin(); itr != Channels[team].end(); itr++)
	{
		if((*itr)->m_channelId == chn->m_channelId)
		{
			Channels[team].erase(itr);
			delete chn;
			return;
		}
	}
