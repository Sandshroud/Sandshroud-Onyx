	...
	case ATTYPE_INSTANCE:
		{
			if(_player->GetPlayerStatus() != TRANSFER_PENDING) //only ports if player is out of pendings
			{
				...
				if( id != 4319 && pMi && ( map->israid() || _player->iInstanceType >= MODE_5PLAYER_NORMAL && pMi->type == INSTANCE_MULTIMODE ) )
				{
					//Do we have a saved instance we should use?
					Instance * in = sInstanceMgr.GetSavedInstance( pMi->mapid,_player->GetLowGUID(), _player->iInstanceType );
