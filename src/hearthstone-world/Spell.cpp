...
int32 GetSpellInfoDuration(SpellEntry* m_spellInfo, Unit* u_caster, Unit* unitTarget)
{
	...
	if(m_spellInfo->DurationIndex)
	{
		...
		if(u_caster)
		{
			...
			{
				SM_FIValue(u_caster->SM[SMT_DURATION][0], (int32*)&m_duration, m_spellInfo->SpellFamilyFlags);
				SM_PIValue(u_caster->SM[SMT_DURATION][1], (int32*)&m_duration, m_spellInfo->SpellFamilyFlags);
			}
		}
...
void SpellCastTargets::read( WorldPacket & data, uint64 caster, uint8 castFlags )
{
	...
	m_srcX = m_srcY = m_srcZ = m_destX = m_destY = m_destZ = traveltime = 0.0f;
	m_strTarget = "";
...
uint64 Spell::GetSinglePossibleEnemy(uint32 i,float prange)
{
	...
	else
	{
		...
		if( GetSpellProto()->SpellFamilyFlags && u_caster)
		{
			SM_FFValue(u_caster->SM[SMT_RADIUS][0],&r,GetSpellProto()->SpellFamilyFlags);
			SM_PFValue(u_caster->SM[SMT_RADIUS][1],&r,GetSpellProto()->SpellFamilyFlags);
		}
	}
...
uint8 Spell::prepare( SpellCastTargets * targets )
{
	...
	if(m_spellInfo->speed > 0.0f && !m_spellInfo->IsChannelSpell() || m_spellInfo->Id == 14157)
	...
	else
	{
		...
		if( m_castTime && GetSpellProto()->SpellFamilyFlags && u_caster != NULL )
		{
			SM_FIValue( u_caster->SM[SMT_CAST_TIME][0], (int32*)&m_castTime, GetSpellProto()->SpellFamilyFlags );
			SM_PIValue( u_caster->SM[SMT_CAST_TIME][1], (int32*)&m_castTime, GetSpellProto()->SpellFamilyFlags );
		}
...
void Spell::AddTime(uint32 type)
{
	if(u_caster && u_caster->IsPlayer())
	{
		...
		{
			float ch = 0;
			SM_FFValue(u_caster->SM[SMT_NONINTERRUPT][1], &ch, GetSpellProto()->SpellFamilyFlags);
			if(Rand(ch))
				return;
		}
	...
	case SPELL_FAILED_REQUIRES_AREA:
		{
			...
			default:	// default case
				Extra = uint32(GetSpellProto()->AreaId);
				break;
			}
		}break;
...
void Spell::SendSpellGo()
{
	...
	ItemPrototype* ip = NULL;
	uint32 cast_flags = SPELL_CAST_FLAGS_NONE;
	if (GetType() == SPELL_DMG_TYPE_RANGED)
...
{
	WorldPacket data(SMSG_RESURRECT_REQUEST, 12+1);
...
bool Spell::HasPower()
{
	...
	if( GetSpellProto()->SpellFamilyFlags && u_caster)
	{
		SM_FIValue(u_caster->SM[SMT_COST][0],&cost,GetSpellProto()->SpellFamilyFlags);
		SM_PIValue(u_caster->SM[SMT_COST][1],&cost,GetSpellProto()->SpellFamilyFlags);
	}
...
uint8 Spell::CanCast(bool tolerate)
{
	...
	if( p_caster != NULL )
	{
		...
		if( GetSpellProto()->RequiredShapeShift )
		{
			...
			{
				if( !(((uint32)1 << (p_caster->GetShapeShift()-1)) & GetSpellProto()->RequiredShapeShift))
		...
		if(i_caster != NULL)
		{
			...
			if(i_caster->GetProto()->Spells[0].Charges != 0)
			{
				...
				if(i_caster->GetUInt32Value(ITEM_FIELD_SPELL_CHARGES) == 0)
				{
					...
					if(i_caster->GetProto()->Class != ITEM_CLASS_MISCELLANEOUS)
		...
		if((!i_caster || (i_caster->GetProto() && i_caster->GetProto()->Flags != 268435520)) && !p_caster->NoReagentCost)
		{
			for(i=0; i<8 ;++i)
			{
		...
		if( GetSpellProto()->AreaId > 0)
		{
			bool found = false;
			uint16 area_id = p_caster->GetAreaID();
			uint32 zone_id = p_caster->GetZoneId();

			AreaTable* table = dbcArea.LookupEntry( GetSpellProto()->AreaId );
			if( table )
			{
				if( table->ZoneId == zone_id || table->AreaId == area_id )
				{
					found = true;
				}
			}
	...
	if( GetSpellProto()->SpellFamilyFlags && u_caster != NULL )
	{
		SM_FFValue( u_caster->SM[SMT_RANGE][0], &maxRange, GetSpellProto()->SpellFamilyFlags );
		SM_PFValue( u_caster->SM[SMT_RANGE][1], &maxRange, GetSpellProto()->SpellFamilyFlags );
	}
	...
	if(m_targets.m_unitTarget)
	{
		...
		if(target != NULL)
		{
			...
			if(target != u_caster && !m_caster->IsPet())
			{
				...
				* 2e way is through the data in the extraspell db
				*
				**********************************************************/
				...
				if(p_caster && GetSpellProto()->FacingCasterFlags)
				{
					...
					if((GetSpellProto()->Flags3 & FLAGS3_REQ_BEHIND_TARGET) && (GetSpellProto()->AttributesEx & ATTRIBUTESEX_REQ_BEHIND) && !p_caster->isInBackOfTarget(target))
					{
...
int32 Spell::CalculateEffect(uint32 i, Unit* target)
{
	...
	float randomPointsPerLevel  = GetSpellProto()->EffectDicePerLevel[i];
	...
	if( u_caster != NULL )
	{
		...
		randomPoints += float2int32(level * randomPointsPerLevel);
	...
	if( caster != NULL )
	{
		...
		SM_FIValue(caster->SM[SMT_MISC_EFFECT][0],&spell_flat_modifers,GetSpellProto()->SpellFamilyFlags);
		SM_FIValue(caster->SM[SMT_MISC_EFFECT][1],&spell_pct_modifers,GetSpellProto()->SpellFamilyFlags);
		switch(i)
		{
		case 0:
			SM_FIValue(caster->SM[SMT_FIRST_EFFECT_BONUS][0],&spell_flat_modifers,GetSpellProto()->SpellFamilyFlags);
			SM_FIValue(caster->SM[SMT_FIRST_EFFECT_BONUS][1],&spell_pct_modifers,GetSpellProto()->SpellFamilyFlags);
			break;
		case 1:
			SM_FIValue(caster->SM[SMT_SECOND_EFFECT_BONUS][0],&spell_flat_modifers,GetSpellProto()->SpellFamilyFlags);
			SM_FIValue(caster->SM[SMT_SECOND_EFFECT_BONUS][1],&spell_pct_modifers,GetSpellProto()->SpellFamilyFlags);
			break;
		case 2:
			SM_FIValue(caster->SM[SMT_LAST_EFFECT_BONUS][0],&spell_flat_modifers,GetSpellProto()->SpellFamilyFlags);
			SM_FIValue(caster->SM[SMT_LAST_EFFECT_BONUS][1],&spell_pct_modifers,GetSpellProto()->SpellFamilyFlags);
			break;
		}
...
void Spell::Heal(int32 amount)
{
	...
	if( u_caster != NULL )
	{
		...
		if(GetSpellProto()->spell_can_crit)
		{
			...
			{
				SM_FFValue(u_caster->SM[SMT_CRITICAL][0], &critchance, GetSpellProto()->SpellFamilyFlags);
				SM_PFValue(u_caster->SM[SMT_CRITICAL][1], &critchance, GetSpellProto()->SpellFamilyFlags);
			}
		}
		...
		if(critical = Rand(critchance))
		{
			...
			if( GetSpellProto()->SpellFamilyFlags )
				SM_FIValue( u_caster->SM[SMT_CRITICAL_DAMAGE][1], &critical_bonus, GetSpellProto()->SpellFamilyFlags );
