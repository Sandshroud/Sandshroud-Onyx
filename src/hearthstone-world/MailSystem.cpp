	...
	if( !msg->items.empty( ) )
	{
		for( itr = msg->items.begin( ); itr != msg->items.end( ); itr++ )
		{
			...
			for( j = 0; j < 7; ++j )
			{
				...
				data << pItem->GetUInt32Value( (ITEM_FIELD_ENCHANTMENT + 1) + ( j * 3 ) );
				data << pItem->GetUInt32Value( (ITEM_FIELD_ENCHANTMENT + 2) + ( j * 3 ) );
			}
...
WorldPacket * Mailbox::MailboxTimePacket()
{
	...
	*data << uint32(0xc7a8c000) << uint32(0);
...
void WorldSession::HandleSendMail(WorldPacket & recv_data )
{
	...
	for( i = 0; i < itemcount; i++ )
	{
		...
		{
			SendMailError(MAIL_ERR_BAG_FULL);
			return;
		}
