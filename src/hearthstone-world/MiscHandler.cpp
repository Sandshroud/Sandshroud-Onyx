...
void WorldSession::HandleRequestAccountData(WorldPacket& recv_data)
{
	...
	WorldPacket data(SMSG_UPDATE_ACCOUNT_DATA, 50);
	data << id;
	...
	else
	{
		if(res->sz > 200)
		{
			...
			ByteBuffer* bbuff = new ByteBuffer(destSize);
...
void WorldSession::HandleSetActionButtonOpcode(WorldPacket& recv_data)
{
	...
	uint8 type = ACTION_BUTTON_TYPE(PackedAction);
	uint32 action = ACTION_BUTTON_ACTION(PackedAction);
...
bool MatchesClass(uint8 _class, uint8 talentTree)
{
	switch(_class)
	{
	case WARRIOR:
		{
			if(talentTree == 161 // Arms
				|| talentTree == 163 // Prot
				|| talentTree == 164) // Fury
				return true;
		}break;
	...
	case HUNTER:
		{
			if(talentTree == 361 // Beast Mastery
				|| talentTree == 363 // Marksmanship
				|| talentTree == 362) // Survival
				return true;
		}break;
	case ROGUE:
		{
			if(talentTree == 182 // Assassination
				|| talentTree == 181 // Combat
				|| talentTree == 183) // Sublety
				return true;
		}break;
	case PRIEST:
		{
			if(talentTree == 201 // Discipline
				|| talentTree == 202 // Holy
				|| talentTree == 203) // Shadow
				return true;
		}break;
	case SHAMAN:
		{
			if(talentTree == 261 // Elemental
				|| talentTree == 263 // Enhancement
				|| talentTree == 262) // Restoration
				return true;
		}break;
	case MAGE:
		{
			if(talentTree == 81 // Arcane
				|| talentTree == 41 // Fire
				|| talentTree == 61) // Frost
				return true;
		}break;
	case WARLOCK:
		{
			if(talentTree == 302 // Affliction
				|| talentTree == 303 // Demonology
				|| talentTree == 301) // Destruction
				return true;
		}break;
	case DRUID:
		{
			if(talentTree == 283 // Balance
				|| talentTree == 281 // Feral
				|| talentTree == 282) // Restoration
				return true;
		}break;
	}
	return false;
...
/*	Player* player = _player->GetMapMgr()->GetPlayer( (uint32)guid );
	...
	uint32 BitMaskLength = 0;
	for(uint8 index = 0; index < 3; index++)
		BitMaskLength += player->TalentMasks[index].GetBlockCount();

	WorldPacket data(SMSG_INSPECT_TALENT, BitMaskLength*4+8+4);
	data << player->GetNewGUID() << BitMaskLength;
	for(uint8 index = 0; index < 3; index++)
		data.append(player->TalentMasks[index].GetMask(), player->TalentMasks[index].GetBlockCount()*4);
	SendPacket( &data );*/
}
