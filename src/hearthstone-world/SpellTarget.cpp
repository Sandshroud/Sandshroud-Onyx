...
uint32 Spell::GetTargetType(uint32 implicittarget, uint32 i)
{
	...
	if(u_caster != NULL)
		SM_FIValue(u_caster->SM[SMT_JUMP_REDUCE][0], (int32*)&jumps, m_spellInfo->SpellFamilyFlags);
	if(jumps != 0)
...
bool Spell::AddTarget(uint32 i, uint32 TargetType, Object* obj)
{
	...
	if(sWorld.Collision && GetDBCMaxRange(r) < 50000 && GetRadius(i) < 50000 && !obj->IsItem())
	{
