...
//		Log.Notice("", "Sent packet %s (0x%03X)", LookupOpcodeName(opcode), uint(opcode), uint(opcode));
	...
	sWorld.LogPacket((uint32)len, opcode, (const uint8*)data, 1);
	...
	_crypt.EncryptSend((uint8*)&Header);
...
void WorldSocket::_HandleAuthSession(WorldPacket* recvPacket)
{
	...
	{
		OutPacket(SMSG_AUTH_RESPONSE, 1, (uint8*)uint8(AUTH_VERSION_MISMATCH));
		return;
	}
...
void WorldSocket::InformationRetreiveCallback(WorldPacket & recvData, uint32 requestid)
{
	...
	if(error != 0 || pAuthenticationPacket == NULL)
	{
		...
		OutPacket(SMSG_AUTH_RESPONSE, 1, (uint8*)uint8(AUTH_FAILED));
		return;
	}
...
//	sLog.outColor(TNORMAL, "\n");
	...
	if( session != NULL )
	{
		...
		OutPacket(SMSG_AUTH_RESPONSE, 1, (uint8*)uint8(AUTH_UNKNOWN_ACCOUNT));
		return;
	}
...
void WorldSocket::UpdateQueuePosition(uint32 Position)
{
	...
	QueuePacket << uint8(AUTH_WAIT_QUEUE) << uint8(0x2C) << uint8(0x73) << uint8(0) << uint8(0);
...
void WorldSocket::OnRecvData()
{
	for(;;)
	{
		...
		if(mRemaining == 0)
		{
			...
			// Decrypt the header
			_crypt.DecryptRecv((uint8*)&Header);
		...
		sWorld.LogPacket(mSize, mOpcode, mSize ? Packet->contents() : NULL, 0);
