...
public:
	MovementInfo()
	{
		flags = flags8 = time = x = y = z = orientation = transX = transY = transZ = transO = 0;
	...
	uint32 FallTime, unk13, unklast;//something related to collision
...
public:
	...
	bool isCasting();
	bool IsInInstance();
	double GetResistanceReduction(Unit* pVictim, uint32 type, float armorReducePct);
	...
	********/
	...
	typedef tr1::unordered_multimap<uint64, pair<ClassFamilyMask, int32> > DamageTakenPctModPerCasterType;
...
//	uint32 fearSpell;
	...
	void SetEquippedItem(uint8 slot, uint32 ItemId);
	uint32 GetEquippedItem(uint8 slot) { return GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_DISPLAY + slot); }
