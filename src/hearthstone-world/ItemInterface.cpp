...
//-------------------------------------------------------------------//
int16 ItemInterface::CanEquipItemInSlot2(int8 DstInvSlot, int8 slot, Item* item, bool ignore_combat /* = false */)
{
	ItemPrototype* proto = item->GetProto();

	if(int8 ret = CanEquipItemInSlot(DstInvSlot, slot, proto, ignore_combat))
		return ret;
...
//-------------------------------------------------------------------//
int16 ItemInterface::CanEquipItemInSlot(int16 DstInvSlot, int16 slot, ItemPrototype* proto, bool ignore_combat /* = false */)
{
	...
	case EQUIPMENT_SLOT_MAINHAND:
		{
			if(proto->Class == ITEM_CLASS_WEAPON && proto->SubClass == ITEM_SUBCLASS_WEAPON_POLEARM)
				return 0;
			if(type == INVTYPE_WEAPON || type == INVTYPE_WEAPONMAINHAND || (type == INVTYPE_2HWEAPON && (!GetInventoryItem(EQUIPMENT_SLOT_OFFHAND) || m_pOwner->ignoreitemreq_cheat)))
				return 0;
			return INV_ERR_ITEM_DOESNT_GO_TO_SLOT;
		}break;
	case EQUIPMENT_SLOT_OFFHAND:
		{
			if(proto->Class == ITEM_CLASS_WEAPON && proto->SubClass == ITEM_SUBCLASS_WEAPON_POLEARM)
				return INV_ERR_ITEM_DOESNT_GO_TO_SLOT;
			if( m_pOwner->ignoreitemreq_cheat && (type == INVTYPE_2HWEAPON || type == INVTYPE_WEAPON || type == INVTYPE_WEAPONOFFHAND || type == INVTYPE_SHIELD))
