				...
				if(Handler->status != STATUS_IGNORED)
				{
					...
					else
					{
						...
						else
						{
							...
							{
								fail = true;
								sLog.outError("Incorrect handling of opcode %s (0x%.4X) REPORT TO DEVS", LookupOpcodeName(packet->GetOpcode()), packet->GetOpcode());
							}
...
void WorldSession::LogoutPlayer(bool Save)
{
	...
	if( _player != NULL )
	{
		...
		_player->RemoveFromBattlegroundQueue(); // Pending BGs
...
void WorldSession::InitPacketHandlerTable()
{
	...
	WorldPacketHandlers[CMSG_OPT_OUT_OF_LOOT].status						= NO_REQ_STATUS; // Sent at login, so no required status form
	...
	WorldPacketHandlers[CMSG_FORCE_FLIGHT_BACK_SPEED_CHANGE_ACK].handler	= &WorldSession::HandleForceSpeedChangeOpcodes;
...
uint8 WorldSession::CheckTeleportPrerequisites(AreaTrigger * pAreaTrigger, WorldSession * pSession, Player* pPlayer, uint32 mapid)
{
	...
	if(pMapInfo->type != INSTANCE_MULTIMODE && pMapInfo->type != INSTANCE_NULL)
		if(pPlayer->iInstanceType >= MODE_5PLAYER_HEROIC)
	...
	if(!pPlayer->triggerpass_cheat)
	{
		...
		if(map->israid())
		{
			...
			if( pPlayer->iInstanceType && pPlayer->getLevel() < 70 )
