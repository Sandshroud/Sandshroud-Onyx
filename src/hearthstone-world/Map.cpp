...
bool CheckResultLengthCreatures(QueryResult * res)
{
	...
	if( fieldcount != 21 )
	{
		if( first_table_warning )
		{
			...
			Log.LargeErrorMessage(LARGERRORMESSAGE_WARNING, format("Your creature_spawns table has the wrong column count(%u,%u).", fieldcount, 21).c_str(),
...
bool CheckResultLengthGameObject(QueryResult * res)
{
	...
	if( fieldcount != 11 )
	{
		if( first_table_warningg )
		{
			...
			Log.LargeErrorMessage(LARGERRORMESSAGE_WARNING, format("Your gameobject_spawns table has the wrong column count(%u,%u).", fieldcount, 11).c_str(),
				"This table has skipped loading in order to avoid crashing.", "Please correct this, if you do not no spawns will show.", NULL);
		}
		...
		if(result)
		{
			if(CheckResultLengthCreatures( result) )
			{
				do
				{
					...
					cspawn->CanMove = fields[19].GetUInt32();
					cspawn->vendormask = fields[20].GetUInt32();
