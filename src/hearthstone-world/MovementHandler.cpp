...
void WorldSession::HandleMovementOpcodes( WorldPacket & recv_data )
{
	...
	if(_player->m_inRangePlayers.size())
	{
		...
		memcpy(&_player->movement_packet[pos], recv_data.contents(), recv_data.size());
		_player->movement_packet[pos+5] = 0;
		...
		for(unordered_set<Player*  >::iterator itr = _player->m_inRangePlayers.begin(); itr != _player->m_inRangePlayers.end(); itr++)
		{
			...
			{
				*(uint32*)&_player->movement_packet[pos+5] = uint32(move_time + (*itr)->GetSession()->m_moveDelayTime);
...
{
	_player->z_axisposition = 0.0f;
	_player->movement_info.init(recvPacket);
}
...
void MovementInfo::init(WorldPacket & data)
{
	...
	if (flags & (MOVEFLAG_SWIMMING | MOVEFLAG_AIR_SWIMMING) || flags8 & 0x20)
	{
		data >> pitch;
	}
...
void MovementInfo::write(WorldPacket & data)
{
	...
	if(flags & MOVEFLAG_FALLING || flags & MOVEFLAG_REDIRECTED)
	{
		data << FallTime << jump_velocity << jump_sinAngle << jump_cosAngle << jump_xySpeed;
	}
	if(flags & MOVEFLAG_SPLINE_MOVER)
	{
		data << spline;
