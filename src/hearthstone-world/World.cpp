...
bool World::SetInitialWorldSettings()
{
	...
	InternalInitialization(start_time);
	return true;
}

void World::InternalInitialization(uint32 start_time)
{
	new ObjectMgr();
	new QuestMgr();
	new LootMgr();
...
#undef MAKE_TASK
	...
	TalentEntry *talent_info = NULL;
	for( DBCStorage<TalentTabEntry>::iterator tab_itr = dbcTalentTab.begin(); tab_itr != dbcTalentTab.end(); ++tab_itr)
	{
		TalentTabEntry *tab_info = (*tab_itr);
		uint32 _class = PlayerMaskToClass(tab_info->ClassMask);
		if(_class == 0)
			continue;

		ClassTalentTrees[_class].insert(make_pair(tab_info->TabPage, tab_info->TalentTabID));

		uint32 count = 0;
		for( DBCStorage<TalentEntry>::iterator talent_itr = dbcTalent.begin(); talent_itr != dbcTalent.end(); ++talent_itr )
		{
			...
			talent_info->BitCounter[0] = count;
			count++;
			for( uint32 j = 1; j < 5; j++ )
			{
				if( talent_info->RankID[j] )
				{
					talent_info->BitCounter[j] = count;
					count++;
				}
			}
		}

		InspectTalentTabSize.insert(make_pair(tab_info->TalentTabID, count));
	}
...
uint32 PlayerMaskToClass(uint32 mask)
{
	if((mask & CLASSMASK_ALL) == 0)
		return 0;
	if(mask & CLASSMASK_WARRIOR)
		return WARRIOR;
	if(mask & CLASSMASK_PALADIN)
		return PALADIN;
	if(mask & CLASSMASK_HUNTER)
		return HUNTER;
	if(mask & CLASSMASK_ROGUE)
		return ROGUE;
	if(mask & CLASSMASK_PRIEST)
		return PRIEST;
	if(mask & CLASSMASK_SHAMAN)
		return SHAMAN;
	if(mask & CLASSMASK_MAGE)
		return MAGE;
	if(mask & CLASSMASK_WARLOCK)
		return WARLOCK;
	if(mask & CLASSMASK_DRUID)
		return DRUID;
	return 0;
};

void World::LogPacket(uint32 len, uint16 opcode, const uint8* data, uint8 direction)
{
	if(!sLog.IsOutProccess())
		return;

	PacketLogMutex.Acquire();
	unsigned int line = 1;
	unsigned int countpos = 0;
	uint16 lenght = len;
	unsigned int count = 0;

	fprintf(m_PacketLog, "{%s} Packet: (0x%04X) %s PacketSize = %u\n", (direction ? "SERVER" : "CLIENT"), opcode,
		LookupOpcodeName(opcode), lenght);
	fprintf(m_PacketLog, "|------------------------------------------------|----------------|\n");
	fprintf(m_PacketLog, "|00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F |0123456789ABCDEF|\n");
	fprintf(m_PacketLog, "|------------------------------------------------|----------------|\n");
	...
	if(lenght > 0)
	{
		...
		for (count = 0 ; count < lenght ; count++)
		{
			...
			fprintf(m_PacketLog, "%02X ",data[count]);
