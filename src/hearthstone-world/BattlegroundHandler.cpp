...
void WorldSession::HandleBattlefieldPortOpcode(WorldPacket &recv_data)
{
	...
	if(action == 0)
		_player->RemoveFromBattlegroundQueue();
	else
	{
		if( GetPlayer()->m_pendingBattleground )
		{
			if(GetPlayer()->m_pendingBattleground->GetType() == bgtype || bgtype == BATTLEGROUND_ARENA_5V5)
			{
				if( GetPlayer()->m_bg )
					GetPlayer()->m_bg->RemovePlayer(GetPlayer(), true); // Send Logout = true so we can TP him now.
				GetPlayer()->m_pendingBattleground->PortPlayer(GetPlayer(), false);
				return;
			}
		}
	}
...
{
	BattlegroundManager.SendBattlegroundQueueStatus(GetPlayer());
}
