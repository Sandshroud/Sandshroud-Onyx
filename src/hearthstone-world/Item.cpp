...
void Item::LoadFromDB(Field* fields, Player* plr, bool light )
{
	...
	if(charter != NULL)
	{
		SetUInt32Value(ITEM_FIELD_ENCHANTMENT, charter->GetID());
...
void Item::ApplyEnchantmentBonus( uint32 Slot, bool Apply )
{
	...
	if(Slot < 1)
	{
		...
		m_owner->SetUInt32Value( PLAYER_VISIBLE_ITEM_1_0 + 1 + Slot + (ItemSlot * PLAYER_VISIBLE_ITEM_LENGTH), Apply ? Entry->Id : 0 );
	}
