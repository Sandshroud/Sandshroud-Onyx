...
pSpellEffect SpellEffectsHandler[TOTAL_SPELL_EFFECTS] = {
		...
		&Spell::SpellEffectTeleportUnits,//SPELL_EFFECT_PORTAL - 12
		...
		&Spell::SpellEffectSummon,//SPELL_EFFECT_SUMMON_WILD - 41
		&Spell::SpellEffectSummon,//SPELL_EFFECT_SUMMON_GUARDIAN - 42
		&Spell::SpellEffectTeleportToCaster,//SPELL_EFFECT_TELEPORT_UNITS_FACE_CASTER - 43
		...
		&Spell::SpellEffectSummon,//SPELL_EFFECT_SUMMON_OBJECT - 50
		...
		&Spell::SpellEffectPull,//SPELL_EFFECT_PULL - 70
		&Spell::SpellEffectPickpocket,//SPELL_EFFECT_PICKPOCKET - 71
		&Spell::SpellEffectAddFarsight,//SPELL_EFFECT_ADD_FARSIGHT - 72
		&Spell::SpellEffectSummon,//SPELL_EFFECT_SUMMON_POSSESSED - 73
		&Spell::SpellEffectSummon,//SPELL_EFFECT_SUMMON_TOTEM - 74
		&Spell::SpellEffectHealMechanical,//SPELL_EFFECT_HEAL_MECHANICAL - 75
		&Spell::SpellEffectSummon,//SPELL_EFFECT_SUMMON_OBJECT_WILD - 76
		...
		&Spell::SpellEffectSummon,//SPELL_EFFECT_SUMMON_TOTEM_SLOT1 - 87
		&Spell::SpellEffectSummon,//SPELL_EFFECT_SUMMON_TOTEM_SLOT2 - 88
		&Spell::SpellEffectSummon,//SPELL_EFFECT_SUMMON_TOTEM_SLOT3 - 89
		&Spell::SpellEffectSummon,//SPELL_EFFECT_SUMMON_TOTEM_SLOT4 - 90
		...
		&Spell::SpellEffectSummon,//SPELL_EFFECT_SUMMON_PHANTASM - 93 OLD
		...
		&Spell::SpellEffectSummon,//SPELL_EFFECT_SUMMON_CRITTER - 97
		...
		&Spell::SpellEffectSummon,//SPELL_EFFECT_SUMMON_DEMON - 112
		...
		&Spell::SpellEffectPull, // SPELL_EFFECT_PULL - 124 - http://thottbot.com/e2312
...
void Spell::SpellEffectHealthLeech(uint32 i) // Health Leech
{
	...
	{
		SM_FFValue(u_caster->SM[SMT_MULTIPLE_VALUE][0], &coef, GetSpellProto()->SpellFamilyFlags);
		SM_PFValue(u_caster->SM[SMT_MULTIPLE_VALUE][1], &coef, GetSpellProto()->SpellFamilyFlags);
	}
...
void Spell::SpellEffectCreateItem(uint32 i) // Create item
{
	...
	if(GetSpellProto()->EffectSpellGroupRelation[i] == 0)
		return;

	SlotResult slotresult;
	ItemPrototype *m_itemProto = ItemPrototypeStorage.LookupEntry( GetSpellProto()->EffectSpellGroupRelation[i] );
	if (!m_itemProto)
		return;
	...
	if(!playerTarget->GetItemInterface()->AddItemById(GetSpellProto()->EffectSpellGroupRelation[i], (item_count > 1 ? item_count : 1 ), m_itemProto->RandomPropId ? m_itemProto->RandomPropId : 0, true, playerTarget))
		return;
...
void Spell::SpellEffectStuck(uint32 i)
{
	...
	sEventMgr.AddEvent(playerTarget,&Player::EventTeleport,mapid,x,y,z,orientation,
...
void Spell::SpellEffectProspecting(uint32 i)
{
	...
	if(p_caster->GetItemInterface()->RemoveItemAmt(entry, 5))
	{
		...
		SendCastResult(SPELL_FAILED_PROSPECT_NEED_MORE);
}
...
void Spell::SpellEffectTranformItem(uint32 i)
{
	...
	if( i_caster == NULL)
		return;
	uint32 itemid=GetSpellProto()->EffectSpellGroupRelation[i];
	if(!itemid)
		return;
...
void Spell::SpellEffectCreateRandomItem(uint32 i) // Create Random Item
{
	...
	itemid = GetSpellProto()->EffectSpellGroupRelation[i];
	m_itemProto = ItemPrototypeStorage.LookupEntry( GetSpellProto()->EffectSpellGroupRelation[i] );
