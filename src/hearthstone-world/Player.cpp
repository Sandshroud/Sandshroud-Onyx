...
void Player::Init()
{
	...
	memset(&mActions, 0, PLAYER_ACTION_BUTTON_COUNT*sizeof(ActionButton));
	tutorialsDirty					= true;
	m_TeleportState					= 1;
	m_beingPushed					= false;
	m_FlyingAura					= 0;
...
bool Player::Create(WorldPacket& data )
{
	...
	if(info == NULL)
	{
		Log.Error("PlayerCreate", "No player create info found for %u %u\n", race, class_);
		return false;
	}
	...
	if(!myRace || !myClass)
	{
		Log.Error("PlayerCreate", "No dbc info found for %u %u\n", race, class_);
		return false;
	}

	m_team = DBC_TEAM_CONVERT(myRace->team_id);
...
void Player::smsg_InitialSpells()
{
	...
	for (sitr = mSpells.begin(); sitr != mSpells.end(); ++sitr)
		data << uint16(*sitr) << uint16(0); // spell id
...
void Player::addSpell(uint32 spell_id)
{
	...
	if(sp2 != NULL && sp2->Id == forget)
	{
		...
		data << uint16(sp2->Id) << uint16(spell_id);
		m_session->SendPacket(&data);
	}
	else
	{
		WorldPacket data(SMSG_LEARNED_SPELL, 6);
		data << uint16(spell_id) << uint16(0);
		m_session->SendPacket(&data);
	}
}
...
void Player::InitVisibleUpdateBits()
{
	...
	for(uint16 i = 0; i < EQUIPMENT_SLOT_END; i++)
	{
		...
		// i am da kreator
		Player::m_visibleUpdateMask.SetBit(PLAYER_VISIBLE_ITEM_1_CREATOR+offset+0);
		Player::m_visibleUpdateMask.SetBit(PLAYER_VISIBLE_ITEM_1_CREATOR+offset+1);

		// entrées
		Player::m_visibleUpdateMask.SetBit((uint16)(PLAYER_VISIBLE_ITEM_1_0+offset)); // visual items for other players

		// chants
		Player::m_visibleUpdateMask.SetBit((uint16)(PLAYER_VISIBLE_ITEM_1_0+1+offset)); // visual items for other players
		Player::m_visibleUpdateMask.SetBit((uint16)(PLAYER_VISIBLE_ITEM_1_0+2+offset)); // visual items for other players
		Player::m_visibleUpdateMask.SetBit((uint16)(PLAYER_VISIBLE_ITEM_1_0+3+offset)); // visual items for other players
		Player::m_visibleUpdateMask.SetBit((uint16)(PLAYER_VISIBLE_ITEM_1_0+4+offset)); // visual items for other players
		Player::m_visibleUpdateMask.SetBit((uint16)(PLAYER_VISIBLE_ITEM_1_0+5+offset)); // visual items for other players
		Player::m_visibleUpdateMask.SetBit((uint16)(PLAYER_VISIBLE_ITEM_1_0+6+offset)); // visual items for other players

		// rand props brah
		Player::m_visibleUpdateMask.SetBit(PLAYER_VISIBLE_ITEM_1_PROPERTIES+offset+0);
		Player::m_visibleUpdateMask.SetBit(PLAYER_VISIBLE_ITEM_1_PROPERTIES+offset+1);

		offset = NULL; // Nullify
	}

	Player::m_visibleUpdateMask.SetBit(PLAYER_CHOSEN_TITLE);
	for(uint32 i = PLAYER_QUEST_LOG_1_1; i < PLAYER_QUEST_LOG_25_2; i += 4)
		Player::m_visibleUpdateMask.SetBit(i);
}
...
void Player::SaveToDB(bool bNewCharacter /* =false */)
{
	...
	for(uint32 i = 0; i < PLAYER_ACTION_BUTTON_COUNT; i++)
		ss << uint32(mActions[i].PackedData) << ",";
	ss << "','";
...
void Player::_LoadTalents(QueryResult * result)
{
	...
	if(result)
	{
		do
		{
			...
			talentRank = fields[1].GetUInt8();
			m_talents.insert(make_pair(talentId, talentRank));
		} while(result->NextRow());
	}
}
...
void Player::_SaveTalentsToDB(QueryBuffer * buf)
{
	...
	if(m_talents.size())
	{
		...
		for(itr = m_talents.begin(); itr != m_talents.end(); itr++)
		{
			...
			ss << "(" << GetLowGUID() << "," << itr->first << "," << uint32(itr->second) << ")";
		}
...
bool Player::LoadFromDB(uint32 guid)
{
	...
	//Talents
	q->AddQuery("SELECT tid, rank FROM playertalents WHERE guid = %u", guid);
...
#define get_next_field fields[field_index++]
	...
	m_bgTeam = m_team = DBC_TEAM_CONVERT(myRace->team_id);
	...
	else
	{
		...
		while(Counter < PLAYER_ACTION_BUTTON_COUNT)
		{
			...
			if(!end)
				break;
			*end = 0;
			mActions[Counter].PackedData = (uint32)atol(start);
			start = end+1;
			Counter++;
		}
	}
...
bool Player::HasTalent(uint32 talentid)
{
	return m_talents.find(talentid) != m_talents.end();
}
...
void Player::_ApplyItemMods(Item* item, int16 slot, bool apply, bool justdrokedown /* = false */, bool skip_stat_apply /* = false  */)
{
	...
	if( setid != 0 )
	{
		...
		if( set != NULL)
		{
			...
			if( apply )
			{
				std::map<uint32, uint32>::iterator itr = m_itemsets.find(setid);
				if(itr == m_itemsets.end())
				{
					m_itemsets.insert(make_pair(setid, 1));
					itr = m_itemsets.find(setid);
				}
				else
					itr->second++;
				...
				if( !set->RequiredSkillID || ( _GetSkillLineCurrent( set->RequiredSkillID, true ) >= set->RequiredSkillAmt ) )
				{
					for( uint8 x = 0; x < 8; x++)
					{
						if( itr->second == set->itemscount[x])
						{//cast new spell
...
{
	WorldPacket data(SMSG_ACTION_BUTTONS, PLAYER_ACTION_BUTTON_COUNT*4);
	for(uint32 slot = 0; slot < PLAYER_ACTION_BUTTON_COUNT; slot++)
		data << mActions[slot].PackedData;
	m_session->SendPacket(&data);
}
...
void Player::setAction(uint8 button, uint32 action, uint8 type)
{
	if( button >= PLAYER_ACTION_BUTTON_COUNT )
		return;
	mActions[button].PackedData = ACTION_PACK_BUTTION(action, type);
}
...
void Player::UpdateChances()
{
	...
	//parry
	tmp = 5.0f + CalcRating( PLAYER_RATING_MODIFIER_PARRY ) + GetParryFromSpell() + defence_contribution;
...
void Player::UpdateStats()
{
	...
	SetFloatValue(PLAYER_FIELD_MOD_MANA_REGEN_INTERRUPT, (regen+stat_regen)*ManaIncreaseRate);
	SetFloatValue(PLAYER_FIELD_MOD_MANA_REGEN, (1.0f+regen+(stat_regen*float(modRegen)/100.0f))*ManaIncreaseRate);
					...
					for(uint32 i = 0; i < pGroup->GetSubGroupCount(); i++)
					{
						for(GroupMembersSet::iterator itr = pGroup->GetSubGroup(i)->GetGroupMembersBegin(); itr != pGroup->GetSubGroup(i)->GetGroupMembersEnd(); itr++)
						{
							...
							{	// If we have pass on, or if we're not in range, we have to pass.
								if( plr->GetSession()->m_passOnLoot || ( !lootObj->IsInRangeSet(plr) ) )
...
int32 Player::CanShootRangedWeapon( uint32 spellid, Unit* target, bool autoshot )
{
	...
	{
		SM_FFValue( SM[SMT_RANGE][0], &maxr, spellinfo->SpellFamilyFlags );
		SM_PFValue( SM[SMT_RANGE][1], &maxr, spellinfo->SpellFamilyFlags );
	}
...
void Player::removeSpellByHashName(uint32 hash)
{
	...
	for(iter= mSpells.begin();iter != mSpells.end();)
	{
		...
		if(e->NameHash == hash)
		{
			...
			WorldPacket data(SMSG_REMOVED_SPELL, 2);
			data << uint16(SpellID);
			SendPacket(&data);
			mSpells.erase(it);
		}
...
void Player::InitializeTalents()
{
	// apply new spec
	uint32 spentPoints = 0;
	if(m_talents.size())
	{
		for(std::map<uint32, uint8>::iterator itr = m_talents.begin(); itr != m_talents.end(); itr++)
		{
...
/*	for(uint8 i = 0; i < 3; i++)
	{
		uint32 tree = sWorld.GetClassTalentTree(i, getClass()), count = sWorld.GetTalentBitCount(tree);
		TalentMasks[i].SetCount(count);
	}*/
}
...
void Player::SendInitialLogonPackets()
{
	...
	*/
	...
	uint32 DayOfTheMonth = 20-1;		//	Day - 1 (0 is actual 1) its now the 20e here.
	uint32 CurrentMonth = 9-1;		//	Month - 1 (0 is actual 1) same as above.
	uint32 CurrentYear = 7;			//	2000 + this number results in a correct value for this crap.
...
{
	std::map<uint32, uint8> *talents = &m_talents;
	std::map<uint32, uint8>::iterator itr;
...
void Player::ApplyTalent(uint32 spellid)
{
	...
	if(HasSpell(spellid))
		return;
	if(spellInfo->RankNumber && FindHigherRankingSpellWithNamehash(spellInfo->NameHash, spellInfo->RankNumber))
		return;
	...
	if( (spellInfo->Attributes & ATTRIBUTES_PASSIVE || (spellInfo->Effect[0] == SPELL_EFFECT_LEARN_SPELL ||
		spellInfo->Effect[1] == SPELL_EFFECT_LEARN_SPELL || spellInfo->Effect[2] == SPELL_EFFECT_LEARN_SPELL) &&
		( ( (spellInfo->c_is_flags & SPELL_FLAG_IS_EXPIREING_WITH_PET) == 0 ) ||
		( ( (spellInfo->c_is_flags & SPELL_FLAG_IS_EXPIREING_WITH_PET) != 0) && GetSummon() ) ) ) )
	{
...
void Player::LearnTalent(uint32 talent_id, uint32 requested_rank)
{
	...
	{
		TalentEntry *depTalentInfo = dbcTalent.LookupEntryForced(talentInfo->DependsOn);
		if (depTalentInfo)
		{
	...
	TalentTabEntry *tab_info = dbcTalentTab.LookupEntry(talentInfo->TalentTree);
//	TalentMasks[tab_info->TabPage].SetBit(talentInfo->BitCounter[requested_rank]);
...
void Player::PlayerRegeneratePower(bool is_interrupted)
{
	...
	for(uint8 power = POWER_TYPE_MANA; power < MAX_POWER_TYPE; power++)
	{
		...
		case POWER_MANA:
			{
				addvalue += GetFloatValue(is_interrupted ? PLAYER_FIELD_MOD_MANA_REGEN_INTERRUPT : PLAYER_FIELD_MOD_MANA_REGEN) * 0.001f * m_regenTimer;
			}break;
...
bool Player::SafeTeleport(uint32 MapID, uint32 InstanceID, LocationVector vec)
{
	...
	if( force_new_world )
	{
		...
		if(mi->flags & WMI_INSTANCE_XPACK_01 && !m_session->HasFlag(ACCOUNT_FLAG_XPACK_01))
		{
	...
	if((GetShapeShift() == FORM_FLIGHT || GetShapeShift() == FORM_SWIFT) && MapID != 530)
...
void Player::RemoveFromBattlegroundQueue(bool forced)
{
	if( m_pendingBattleground )
	{
		m_pendingBattleground->RemovePendingPlayer(TO_PLAYER(this));
		m_pendingBattleground = NULLBATTLEGROUND;
	}

	m_bgIsQueued = false;
	m_bgQueueType = 0;
	m_bgQueueInstanceId = 0;
	BattlegroundManager.SendBattlegroundQueueStatus(TO_PLAYER(this));
	sEventMgr.RemoveEvents(TO_PLAYER(this), EVENT_BATTLEGROUND_QUEUE_UPDATE);
...
void Player::Cooldown_Add(SpellEntry * pSpell, Item* pItemCaster)
{
	...
	if( pSpell->CategoryRecoveryTime > 0 && pSpell->Category )
	{
		...
		if( pSpell->SpellFamilyFlags )
		{
			SM_FIValue(SM[SMT_COOLDOWN_DECREASE][0], &cool_time, pSpell->SpellFamilyFlags);
			SM_PIValue(SM[SMT_COOLDOWN_DECREASE][1], &cool_time, pSpell->SpellFamilyFlags);
		}
...
void Player::Cooldown_AddStart(SpellEntry * pSpell)
{
	...
	if( pSpell->SpellFamilyFlags )
	{
		SM_FIValue(SM[SMT_GLOBAL_COOLDOWN][0], &atime, pSpell->SpellFamilyFlags);
		SM_PIValue(SM[SMT_GLOBAL_COOLDOWN][1], &atime, pSpell->SpellFamilyFlags);
	}
...
void Player::VampiricSpell(uint32 dmg, Unit* pTarget, SpellEntry *spellinfo)
{
	...
	{
		int32 pct = 15;
		uint32* sgt = new uint32[2]; sgt[0] = 4; sgt[1] = 0;
		ClassFamilyMask mask; mask.Flags = uint64(sgt);
		SM_PIValue(SM[SMT_MISC_EFFECT][1], &pct, mask);
...
uint32 Player::GetMaxPersonalRating()
{
	...
	uint32 maxrating = 0;
	for (uint i = ARENA_TEAM_TYPE_2V2; i < NUM_ARENA_TEAM_TYPES; i++)
	{
...
void Player::SetKnownTitle( int32 title, bool set )
{
	...
	if( set )
		SetUInt64Value( field, current | uint64(1) << title );
	else
		SetUInt64Value( field, current & ~uint64(1) << title );
...
uint16 Player::FindQuestSlot( uint32 questid )
{
	for(uint16 i = 0; i < 25; i++)
		if( (GetUInt32Value(PLAYER_QUEST_LOG_1_1 + i * 4)) == questid )
			return i;
