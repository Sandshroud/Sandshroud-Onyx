...
#define DBC_TEAM_CONVERT(x) (x == 7 ? 0 : 1)
...
enum ClassMasks
{
	...
	CLASSMASK_ALL			= (CLASSMASK_WARRIOR | CLASSMASK_PALADIN | CLASSMASK_HUNTER |
								CLASSMASK_ROGUE | CLASSMASK_PRIEST | CLASSMASK_SHAMAN |
								CLASSMASK_MAGE | CLASSMASK_WARLOCK | CLASSMASK_DRUID) // ?
};
...
static uint32 ItemRatingValues[EXPERTISE_RATING+1] = {
	0, // POWER
	0, // HEALTH
	0, // UNK
	0, // AGILITY
	...
	PLAYER_RATING_MODIFIER_EXPERTISE, // EXPERTISE_RATING_0
...
public:
	...
	uint32 GetQuestSlotQuestId(uint16 slot) const { return GetUInt32Value(PLAYER_QUEST_LOG_1_1 + slot * 4); }
	...
	HEARTHSTONE_INLINE void ResetTeam() { m_team = DBC_TEAM_CONVERT(myRace->team_id); m_bgTeam=m_team; }
...
//	UpdateMask TalentMasks[3];
	std::map<uint32, uint8> m_talents;	// map of <talentId, talentRank>
	...
	******************/
	...
	void RemoveFromBattlegroundQueue(bool forced = false);
	void FullHPMP();
...
protected:
	...
	std::map<uint32, uint32> m_itemsets;
