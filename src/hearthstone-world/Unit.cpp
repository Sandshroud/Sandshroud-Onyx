	...
	for( itr = m_procSpells.begin(); itr != m_procSpells.end(); )  // Proc Trigger Spells for Victim
	{
		...
		if( (flag && (itr2->procFlags & flag)) || (flag2 && (itr2->procflags2 & flag2)) ) // Check both.
		{
			...
			if( itr2->procFlags & PROC_ON_CAST_SPELL || itr2->procFlags & PROC_ON_SPELL_LAND || itr2->procFlags & PROC_ON_CAST_SPECIFIC_SPELL || itr2->procFlags & PROC_ON_ANY_HOSTILE_ACTION || (itr2->procFlags & PROC_ON_PHYSICAL_ATTACK && sp->Spell_Dmg_Type & SPELL_DMG_TYPE_MELEE))
			{
				...
				if( itr2->FamilyFlags )
				{
					if(!CastingSpell->SpellFamilyFlags.IsFitToFamilyMask(itr2->FamilyFlags))
						continue;
				}
			...
			if(ospinfo != NULL)
				SM_FIValue( SM[SMT_PROC_CHANCE][0], (int32*)&proc_Chance, ospinfo->SpellFamilyFlags );
						...
						case 12494: //Frostbite
							{
								...
								if( !( CastingSpell->SpellFamilyFlags.IsFitToFamilyMask(0x100220) ) || victim == TO_UNIT(this) ) //frost
									continue;
							}break;
	...
	for(i = m_damageShields.begin();i != m_damageShields.end();)     // Deal Damage to Attacker
	{
		...
		if(	((*i2).m_flags) & flag )
		{
			if( (*i2).destination )
			{
				...
				data << (*i2).m_spellId;
				data << (*i2).m_damage;
				data << uint32(SchoolMask((*i2).m_school));
...
double Unit::GetResistanceReduction(Unit* pVictim, uint32 school, float armorReducePct)
{
...
void Unit::CalculateResistanceReduction(Unit* pVictim,dealdamage * dmg, SpellEntry* ability, float armorReducePct)
{
	...
	double reduction = GetResistanceReduction(pVictim, dmg->school_type, armorReducePct);
...
//--------------------------------parry chance----------------------------------------------
	...
	else if(pVictim->IsCreature())
	{
		...
		if( !backAttack )
		{
			if( c->b_has_shield && pVictim->GetUInt32Value(UNIT_FIELD_STRENGTH))
	...
	{
		SM_FFValue( SM[SMT_HITCHANCE][0], &hitchance, ability->SpellFamilyFlags );
		SM_PFValue( SM[SMT_HITCHANCE][1], &hitchance, ability->SpellFamilyFlags );
	}
...
uint32 Unit::GetSpellDidHitResult( uint32 index, Unit* pVictim, Spell* pSpell, uint8&reflectout )
{
	...
	if(m_spellEntry->SpellFamilyFlags)
	{
		SM_FFValue(SM[SMT_HITCHANCE][0], &hitchance, m_spellEntry->SpellFamilyFlags);
		SM_PFValue(SM[SMT_HITCHANCE][1], &hitchance, m_spellEntry->SpellFamilyFlags);
	}
	...
	if( m_spellEntry->Effect[index] == SPELL_EFFECT_DISPEL && m_spellEntry->SpellFamilyFlags)
	{
		SM_FFValue(pVictim->SM[SMT_RESIST_DISPEL][0], &resistchance, m_spellEntry->SpellFamilyFlags);
		SM_PFValue(pVictim->SM[SMT_RESIST_DISPEL][1], &resistchance, m_spellEntry->SpellFamilyFlags);
	}

	if(Spell::IsBinary(m_spellEntry))
	{ // need to apply resistance mitigation
		float mitigation = 1.0f - float (GetResistanceReduction(pVictim, m_spellEntry->School, 0.0f));
...
//--------------------------------by damage type and by weapon type-------------------------
	...
	if(ability && ability->SpellFamilyFlags)
	{
		SM_FFValue(SM[SMT_CRITICAL][0],&crit,ability->SpellFamilyFlags);
		SM_PFValue(SM[SMT_CRITICAL][1],&crit,ability->SpellFamilyFlags);
		SM_FFValue(SM[SMT_HITCHANCE][0],&hitchance,ability->SpellFamilyFlags);
		SM_PFValue(SM[SMT_HITCHANCE][1],&hitchance,ability->SpellFamilyFlags);
	}
	...
	if( ability != NULL )
	{
		if( pVictim->IsStunned() && ability->Id == 20467 )
			crit = 100.0f;
			...
			case 5:
				{
					...
					if(ability && ability->SpellFamilyFlags)
						SM_FFValue(SM[SMT_CRITICAL_DAMAGE][1],&dmg_bonus_pct,ability->SpellFamilyFlags);
...
//--------------------------extra strikes processing----------------------------------------
	...
	/*if( ability && ability->SpellFamilyFlags )
	{
		SM_FIValue(SM[SMT_ADDITIONAL_TARGET][0], &m_extraattacks, ability->SpellFamilyFlags);
	}*/
...
int32 Unit::GetSpellBonusDamage(Unit* pVictim, SpellEntry *spellInfo,int32 base_dmg, bool healing)
{
	...
	{
		float modifier = 0;
		SM_FFValue( caster->SM[SMT_SP_BONUS][0], &modifier, spellInfo->SpellFamilyFlags );
		SM_FFValue( caster->SM[SMT_SP_BONUS][1], &modifier, spellInfo->SpellFamilyFlags );
	...
	if( spellInfo->SpellFamilyFlags )
	{
		...
		SM_FIValue(caster->SM[SMT_DAMAGE_DONE][0], &bonus_damage, spellInfo->SpellFamilyFlags);
		SM_FFValue(caster->SM[SMT_SPELL_VALUE_PCT][1], &dmg_bonus_pct, spellInfo->SpellFamilyFlags);
		SM_FFValue(caster->SM[SMT_DAMAGE_DONE][1], &dmg_bonus_pct, spellInfo->SpellFamilyFlags);
...
void Unit::RemoveFromWorld(bool free_guid)
{
	...
	m_AuraInterface.RemoveAllAurasWithSpEffect(SPELL_EFFECT_UNKNOWN7);
...
void Unit::SetEquippedItem(uint8 slot, uint32 ItemId)
{
	if (ItemId == 0)
	{
	...
	SetByte(UNIT_VIRTUAL_ITEM_INFO + (slot * 2) + 0, VIRTUAL_ITEM_INFO_0_OFFSET_CLASS, proto->Class);
	SetByte(UNIT_VIRTUAL_ITEM_INFO + (slot * 2) + 0, VIRTUAL_ITEM_INFO_0_OFFSET_SUBCLASS, proto->SubClass);
	SetByte(UNIT_VIRTUAL_ITEM_INFO + (slot * 2) + 0, VIRTUAL_ITEM_INFO_0_OFFSET_UNK0, proto->unknown_bc);
	SetByte(UNIT_VIRTUAL_ITEM_INFO + (slot * 2) + 0, VIRTUAL_ITEM_INFO_0_OFFSET_MATERIAL, proto->LockMaterial);
	SetByte(UNIT_VIRTUAL_ITEM_INFO + (slot * 2) + 1, VIRTUAL_ITEM_INFO_1_OFFSET_INVENTORYTYPE, proto->InventoryType);
	SetByte(UNIT_VIRTUAL_ITEM_INFO + (slot * 2) + 1, VIRTUAL_ITEM_INFO_1_OFFSET_SHEATH, proto->SheathId);
}
...
{
	if(IsPlayer())
		TO_PLAYER(this)->SafeTeleport(GetMapId(), GetInstanceID(), x, y, z, o);
	else
	{
