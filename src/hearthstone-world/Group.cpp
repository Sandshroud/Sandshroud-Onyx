...
{
	GROUP_UPDATE_FLAG_NONE						= 0x00000000,	// 0x00000000
	GROUP_UPDATE_FLAG_ONLINE					= 0x00000001,	// 0x00000001  uint8
	GROUP_UPDATE_FLAG_HEALTH					= 0x00000002,	// 0x00000002  uint16
	GROUP_UPDATE_FLAG_MAXHEALTH					= 0x00000004,	// 0x00000004  uint16
	GROUP_UPDATE_FLAG_POWER_TYPE				= 0x00000008,	// 0x00000008  uint16
	GROUP_UPDATE_FLAG_POWER						= 0x00000010,	// 0x00000010  uint16
	GROUP_UPDATE_FLAG_MAXPOWER					= 0x00000020,	// 0x00000020  uint16
	GROUP_UPDATE_FLAG_LEVEL						= 0x00000040,	// 0x00000040  uint16
	GROUP_UPDATE_FLAG_ZONEID					= 0x00000080,	// 0x00000080  uint16
	GROUP_UPDATE_FLAG_POSITION					= 0x00000100,	// 0x00000100  uint16, uint16
	GROUP_UPDATE_FLAG_PLAYER_AURAS				= 0x00000200,	// 0x00000200  uint64, uint16 for each uint64
	GROUP_UPDATE_FLAG_PET_GUID					= 0x00000400,	// 0x00000400  uint64
	GROUP_UPDATE_FLAG_PET_NAME					= 0x00000800,	// 0x00000800  string
	GROUP_UPDATE_FLAG_PET_DISPLAYID				= 0x00001000,	// 0x00001000  uint16
	GROUP_UPDATE_FLAG_PET_HEALTH				= 0x00002000,	// 0x00002000  uint16
	GROUP_UPDATE_FLAG_PET_MAXHEALTH				= 0x00004000,	// 0x00004000  uint16
	GROUP_UPDATE_FLAG_PET_POWER_TYPE			= 0x00008000,	// 0x00008000  uint8
	GROUP_UPDATE_FLAG_PET_POWER					= 0x00010000,	// 0x00010000  uint16
	GROUP_UPDATE_FLAG_PET_MAXPOWER				= 0x00020000,	// 0x00020000  uint16
	GROUP_UPDATE_FLAG_PET_AURAS					= 0x00040000,	// 0x00040000  uint64, uint16 for each uint64

	GROUP_UPDATE_PET							= GROUP_UPDATE_FLAG_PET_GUID | GROUP_UPDATE_FLAG_PET_NAME | GROUP_UPDATE_FLAG_PET_DISPLAYID | \
	GROUP_UPDATE_FLAG_PET_HEALTH | GROUP_UPDATE_FLAG_PET_MAXHEALTH | GROUP_UPDATE_FLAG_PET_POWER_TYPE | GROUP_UPDATE_FLAG_PET_POWER | GROUP_UPDATE_FLAG_PET_MAXPOWER | \
	GROUP_UPDATE_FLAG_PET_AURAS,	// 0x0007FFFF

	GROUP_UPDATE_FULL							= GROUP_UPDATE_FLAG_ONLINE | GROUP_UPDATE_FLAG_HEALTH | GROUP_UPDATE_FLAG_MAXHEALTH | \
	GROUP_UPDATE_FLAG_POWER_TYPE | GROUP_UPDATE_FLAG_POWER | GROUP_UPDATE_FLAG_MAXPOWER | GROUP_UPDATE_FLAG_LEVEL | GROUP_UPDATE_FLAG_ZONEID | \
	GROUP_UPDATE_FLAG_POSITION | GROUP_UPDATE_FLAG_PLAYER_AURAS,	// 0x0007FFFF
...
void Group::Update()
{
	...
	PlayerInfo *plrinf;
	WorldPacket data(35 + ( m_MemberCount * (10 + 5) ) );
	GroupMembersSet::iterator itr1, itr2;
	uint32 i = 0, j = 0; uint8 flags;
				...
				for( j = 0; j < m_SubGroupCount; j++ )
				{
					...
					if( sg2 != NULL)
					{
						for( itr2 = sg2->GetGroupMembersBegin(); itr2 != sg2->GetGroupMembersEnd(); itr2++ )
						{
							...
							flags = 0;
							if(m_assistantLeader &&  (*itr2)->guid == m_assistantLeader->guid)
								flags |= 1;
							if(m_mainTank && (*itr2)->guid == m_mainTank->guid )
								flags |= 2;
							if(m_mainAssist && (*itr2)->guid == m_mainAssist->guid )
								flags |= 4;
							data << (*itr2)->name;
							data << (uint32)(*itr2)->guid << uint32(0);
							data << uint8( ((*itr2)->m_loggedInPlayer ? 1 : 0) );
							data << uint8( sg2->GetID() ) << flags;
						}
					}
				}

				data << uint32( (m_Leader ? m_Leader->guid : 0) ) << uint32(0);
				data << uint8( m_LootMethod );
				data << uint32( (m_Looter ? m_Looter->guid : 0)) << uint32(0);
...
void SubGroup::Disband()
{
	...
	WorldPacket data2(SMSG_PARTY_COMMAND_RESULT, 9);
...
void Group::UpdateOutOfRangePlayer(Player* pPlayer, uint32 Flags, bool Distribute, WorldPacket * Packet)
{
	...
	if(pPlayer->GetPowerType() != POWER_TYPE_MANA)
		Flags |= GROUP_UPDATE_FLAG_POWER_TYPE | GROUP_UPDATE_FLAG_POWER | GROUP_UPDATE_FLAG_MAXPOWER;
	...
	*data << Flags;
	...
	{
		if (pPlayer->GetSummon() != NULL)
			*data << uint16(pPlayer->GetSummon()->GetUInt32Value(UNIT_FIELD_HEALTH));
		else
			*data << uint16(0);
	}
	...
	{
		if (pPlayer->GetSummon() != NULL)
			*data << uint16(pPlayer->GetSummon()->GetUInt32Value(UNIT_FIELD_MAXHEALTH));
		else
			*data << uint16(0);
	}
...
void Group::UpdateAllOutOfRangePlayersFor(Player* pPlayer)
{
	...
	WorldPacket data(150), data2(150);

	/* tell the other players about us */
	UpdateOutOfRangePlayer(pPlayer, GROUP_UPDATE_FULL, true, &data2);
	...
	for(uint32 i = 0; i < m_SubGroupCount; i++)
	{
		...
		for(GroupMembersSet::iterator itr = m_SubGroups[i]->GetGroupMembersBegin(); itr != m_SubGroups[i]->GetGroupMembersEnd(); itr++)
		{
			...
			{
				UpdateOutOfRangePlayer(plr, GROUP_UPDATE_FULL, false, &data);
				pPlayer->GetSession()->SendPacket(&data);
			}
		}
	}
...
void WorldSession::HandlePartyMemberStatsOpcode(WorldPacket & recv_data)
{
	...
	_player->GetGroup()->UpdateOutOfRangePlayer(plr, GROUP_UPDATE_FULL | GROUP_UPDATE_TYPE_FULL_REQUEST_REPLY, false, &data);
