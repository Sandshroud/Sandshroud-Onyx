...
CBattlegroundManager::CBattlegroundManager() : EventableObject(), m_queueLock(), m_instanceLock()
{
...
void CBattlegroundManager::HandleBattlegroundJoin(WorldSession * m_session, WorldPacket & pck)
{
	...
	if( plr->m_pendingBattleground )
		plr->m_pendingBattleground->RemovePendingPlayer(plr);
	...
	plr->m_bgQueueTime = (uint32)UNIXTIME;
	...
	SendBattlegroundQueueStatus( plr );
...
void CBattlegroundManager::EventQueueUpdate(bool forceStart)
{
	...
	m_queueLock.Acquire();
	m_instanceLock.Acquire();
	for(uint32 i = 0; i < BATTLEGROUND_NUM_TYPES; i++)
	{
		for(uint32 j = 0; j < MAX_LEVEL_GROUP; ++j)
		{
			...
			for(list<uint32>::iterator it3 = m_queuedPlayers[i][j].begin(), it4; it3 != m_queuedPlayers[i][j].end();)
			{
				it4 = it3++;
				...
				if(plr->m_bgQueueInstanceId != 0 && plr->m_bgIsQueued)
				{
					if( m_instances[i].empty() )
						continue;

					map<uint32, CBattleground* >::iterator iitr = m_instances[i].find( plr->m_bgQueueInstanceId );
					if(iitr == m_instances[i].end())
					{
						...
						plr->RemoveFromBattlegroundQueue();
						//m_queuedPlayers[i][j].erase(it4);
						SendBattlegroundQueueStatus(plr);
						continue;
					}
			...
			for(map<uint32, CBattleground* >::iterator iitr = m_instances[i].begin(); iitr != m_instances[i].end(); iitr++)
			{
	...
	for(uint32 i = BATTLEGROUND_ARENA_2V2; i <= BATTLEGROUND_ARENA_5V5; i++)
	{
...
void CBattlegroundManager::RemovePlayerFromQueues(Player* plr)
{
	...
	uint32 lgroup = GetLevelGrouping(plr->getLevel());
	list<uint32>::iterator itr = m_queuedPlayers[plr->m_bgQueueType][lgroup].begin(), itr2;
	while(itr != m_queuedPlayers[plr->m_bgQueueType][lgroup].end())
	{
		...
		{
			OUT_DEBUG("BattlegroundManager", "Removing player %u from queue instance %u type %u", plr->GetLowGUID(), plr->m_bgQueueInstanceId, plr->m_bgQueueType);
	...
	plr->m_pendingBattleground = NULLBATTLEGROUND;
	plr->m_bgQueueType = 0;
...
void CBattlegroundManager::RemoveGroupFromQueues(Group * grp)
{
	...
	for(GroupMembersSet::iterator itr = grp->GetSubGroup(0)->GetGroupMembersBegin(); itr != grp->GetSubGroup(0)->GetGroupMembersEnd(); itr++)
	{
		...
		{
			(*itr)->m_loggedInPlayer->m_bgIsQueued = false;
			SendBattlegroundQueueStatus((*itr)->m_loggedInPlayer);
		}
	}
...
void CBattleground::AddPlayer(Player* plr, uint32 team)
{
	...
	sEventMgr.AddEvent(plr, &Player::RemoveFromBattlegroundQueue, true, EVENT_BATTLEGROUND_QUEUE_UPDATE, 120000, 1,0);

	plr->m_pendingBattleground = this;
	plr->m_bgIsQueued = false;

	/* Send a packet telling them that they can enter */
	BattlegroundManager.SendBattlegroundQueueStatus(plr);
...
void CBattleground::RemovePendingPlayer(Player* plr)
{
	...
	if( plr->m_pendingBattleground && plr->m_pendingBattleground == this )
	{
		if( plr->m_pendingBattleground->IsArena() )
...
void CBattleground::PortPlayer(Player* plr, bool skip_teleport /* = false*/)
{
	...
	if(m_ended)
	{
		if( plr->m_pendingBattleground == this )
		{
			plr->m_pendingBattleground = NULLBATTLEGROUND;
			plr->m_bgIsQueued = false;
		}
...
void CBattlegroundManager::SendBattlegroundQueueStatus(Player* plr)
{
	...
	if( plr->m_bg )
	{
		...
		data << uint32(plr->m_bg->GetType());
		data << uint16(0x1F90);
		data << uint32(plr->m_bg->GetMapMgr()->GetInstanceID());
		data << uint8(plr->m_bgTeam);
		data << uint32(3);
		data << plr->m_bg->GetMapMgr()->GetMapId();
		data << uint32(0);
		data << uint32(UNIXTIME-plr->m_bg->GetStartTime());
		data << uint8(IS_ARENA(plr->m_bgQueueType) ? 0 : 1);
		plr->GetSession()->SendPacket(&data);
		return;
	}

	// We're no longer queued!
	if( !plr->m_bgIsQueued && !plr->m_pendingBattleground)
	{
	...
	{
		data << uint32(2);
		data << plr->m_pendingBattleground->GetMapMgr()->GetMapId();
		data << uint32(0); // Time
		plr->GetSession()->SendPacket(&data);
		return;
	}
	...
	data << uint32(60);		// average time in msec
	data << uint32(0);
	plr->GetSession()->SendPacket(&data);
}
	...
	for(i = m_resurrectMap.begin(); i != m_resurrectMap.end(); i++)
	{
		for(itr = i->second.begin(); itr != i->second.end(); itr++)
		{
			...
			if(plr && plr->isDead())
			{
				...
				data << plr->GetNewGUID() << plr->GetNewGUID() << uint32(RESURRECT_SPELL) << uint8(0) << uint8(1) << uint8(1) << plr->GetGUID() << uint8(0) << uint16(2) << plr->GetGUID();
				plr->SendMessageToSet(&data, true);
...
void CBattlegroundManager::HandleArenaJoin(WorldSession * m_session, uint32 BattlegroundType, uint8 as_group, uint8 rated_match)
{
		...
		else
		{
			...
			for(itx = pGroup->GetSubGroup(0)->GetGroupMembersBegin(); itx != pGroup->GetSubGroup(0)->GetGroupMembersEnd(); itx++)
			{
				if((*itx)->m_loggedInPlayer)
				{
					...
					(*itx)->m_loggedInPlayer->m_bgQueueTime = (uint32)UNIXTIME;
					...
					SendBattlegroundQueueStatus( (*itx)->m_loggedInPlayer);
				}
			}
	...
	plr->m_bgIsQueued = true;
	plr->m_bgQueueInstanceId = 0;
	plr->m_bgQueueType = BattlegroundType;
	plr->m_bgRatedQueue = false;
	plr->m_bgQueueTime = (uint32)UNIXTIME;
	SendBattlegroundQueueStatus(plr);
