...
void ObjectMgr::LoadPlayersInfo()
{
	...
	if(result)
	{
		...
		do
		{
			...
			pn->team = DBC_TEAM_CONVERT(race->team_id);
...
void ObjectMgr::LoadPetLevelupSpellMap()
{
	...
	for	(uint32	i =	0; i < dbcCreatureFamily.GetNumRows(); i++)
	{
		...
		for(uint32 j = 0; j < dbcSkillLineSpell.GetNumRows(); ++j)
		{
			...
			case SKILL_PET_IMP:
				{
					mPetLevelupSpellMap[FAMILY_IMP][sp->spellLevel] = sk->spell;
				}break;
			case SKILL_PET_VOIDWALKER:
				{
					mPetLevelupSpellMap[FAMILY_VOIDWALKER][sp->spellLevel] = sk->spell;
				}break;
			case SKILL_PET_SUCCUBUS:
				{
					mPetLevelupSpellMap[FAMILY_SUCCUBUS][sp->spellLevel] = sk->spell;
				}break;
			case SKILL_PET_FELHUNTER:
				{
					mPetLevelupSpellMap[FAMILY_FELHUNTER][sp->spellLevel] = sk->spell;
				}break;
			case SKILL_PET_FELGUARD:
				{
					mPetLevelupSpellMap[FAMILY_FELGUARD][sp->spellLevel] = sk->spell;
				}break;
