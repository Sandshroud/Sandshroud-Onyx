...
pSpellAura SpellAuraHandler[TOTAL_SPELL_AURAS] = {
	...
	&Aura::SpellAuraIgnore,//SPELL_AURA_EXTRA_ATTACKS = 70,//obselete?
	...
	&Aura::SpellAuraNULL/*SpellAuraModAOEDodge*/,//missing = 160 //Apply Aura: Mod Side/Rear PBAE Damage Taken %//used http://www.thottbot.com/?sp=23198
	...
	&Aura::SpellAuraNULL/*SpellAuraAxeSkillModifier*/,//240 Increase Axe Skill http://www.wowhead.com/?spell=20574
};
...
Aura::Aura( SpellEntry* proto, int32 duration, Object* caster, Unit* target )
{
	...
	if( caster->IsUnit() )
	{
		...
		{
			SM_FIValue(TO_UNIT(caster)->SM[SMT_CHARGES][0], (int32*)&procCharges, m_spellProto->SpellFamilyFlags);
			SM_PIValue(TO_UNIT(caster)->SM[SMT_CHARGES][1], (int32*)&procCharges, m_spellProto->SpellFamilyFlags);
		}
...
void Aura::Remove()
{
	...
	for( uint32 x = 0; x < 3; x++ )
	{
		...
		else if( (m_spellProto->Effect[x] == SPELL_EFFECT_APPLY_AREA_AURA || m_spellProto->Effect[x] == SPELL_EFFECT_UNKNOWN7) && m_casterGuid == m_target->GetGUID())
			RemoveAA();
	}

	int32 proccharges = m_spellProto->procCharges;
	if( m_caster != NULL && m_spellProto->SpellFamilyFlags )
	{
		SM_FIValue(m_caster->SM[SMT_CHARGES][0],&proccharges, m_spellProto->SpellFamilyFlags);
		SM_PIValue(m_caster->SM[SMT_CHARGES][1],&proccharges, m_spellProto->SpellFamilyFlags);
	}
...
void Aura::BuildAuraUpdate()
{
	...
	uint8 flagslot = (m_auraSlot / 4);
	uint32 bit = (m_auraSlot % 4) * 8;
	{	// AURAFLAGS
	...
	m_target->SetUInt32Value(UNIT_FIELD_AURA + m_auraSlot, (m_deleted ? 0 : spellid));

	{	// AURALEVELS
		uint32 val = m_target->GetUInt32Value(UNIT_FIELD_AURALEVELS + flagslot);
		val &= ~(0xFF << bit);
		if(!m_deleted)
		{
			if(positive)
		...
		m_target->SetUInt32Value(UNIT_FIELD_AURALEVELS + flagslot, val);
	}

	{
		uint32 ac = stackSize ? stackSize-1 : 0;
		uint32 val = m_target->GetUInt32Value(UNIT_FIELD_AURAAPPLICATIONS+flagslot);
		val &= ~(0xFF << bit);
		if(!m_deleted)
			val |= (ac << bit);
		m_target->SetUInt32Value(UNIT_FIELD_AURAAPPLICATIONS+flagslot, val);
	}
	...
	data.Initialize(SMSG_INIT_EXTRA_AURA_INFO, 22);
	data << m_target->GetNewGUID() << m_auraSlot << uint32(m_spellProto->Id) << uint32(GetDuration()) << uint32(GetTimeLeft());
	...
	if( m_casterGuid && m_target->GetGUID() != m_casterGuid )//don't use resist when cast on self-- this is some internal stuff
	{
		...
		if( m_caster != NULL )
		{
			...
			if( DOTCanCrit == true )
			{
				...
				if( m_spellProto->SpellFamilyFlags )
				{
					SM_FFValue(m_caster->SM[SMT_CRITICAL][0], &CritChance, m_spellProto->SpellFamilyFlags);
					SM_PFValue(m_caster->SM[SMT_CRITICAL][1], &CritChance, m_spellProto->SpellFamilyFlags);
				}
				...
				if (critical)
				{
					...
					if( m_spellProto->SpellFamilyFlags )
						SM_FIValue( m_caster->SM[SMT_CRITICAL_DAMAGE][1], &critical_bonus, m_spellProto->SpellFamilyFlags );
	...
	case 71903: // Shadowmourne Weapon Effect
		{
			if(m_caster != NULL)
			{
				if(apply)
				{
					CreateProcTriggerSpell(m_caster, m_caster->GetGUID(), GetSpellId(), 71905, mod->m_amount, PROC_ON_MELEE_ATTACK | PROC_ON_PHYSICAL_ATTACK | PROC_ON_CRIT_ATTACK, PROC_TARGET_SELF, NULL);
				}
				else
				{
	...
	case 63373:
		{
			if(m_caster != NULL)
			{
				if(apply)
				{
					CreateProcTriggerSpell(m_caster, m_caster->GetGUID(), GetSpellId(), 63685, mod->m_amount, PROC_ON_CAST_SPELL, NULL, NULL);
				}
				else
				{
					for(std::list<struct ProcTriggerSpell>::iterator itr = m_target->m_procSpells.begin();itr != m_target->m_procSpells.end();itr++)
	...
	case 51558:
		{
			if(m_caster != NULL)
			{
				if(apply)
				{
					CreateProcTriggerSpell(m_caster, m_caster->GetGUID(), GetSpellId(), 52752, mod->m_amount, PROC_ON_SPELL_CRIT_HIT, NULL, NULL);
				}
				else
				{
					for(std::list<struct ProcTriggerSpell>::iterator itr = m_target->m_procSpells.begin();itr != m_target->m_procSpells.end();itr++)
			...
			if(m_caster != NULL && m_caster->IsPlayer())
			{
				...
				if(apply)
				{
					...
					if(mod->i == 2)
					{
						...
						CreateProcTriggerSpell(plr, plr->GetGUID(), GetSpellId(), spellid, procchance, PROC_ON_MELEE_ATTACK, NULL, NULL);
				}
				else
				{
					if(mod->i == 2)
	...
	case 25899: // Greater Blessing of Sanctuary
		{
			...
			if(apply)
			{
				...
				CreateProcTriggerSpell(m_target, m_target->GetGUID(), GetSpellId(), spellid, 100, PROC_ON_ABSORB|PROC_ON_MELEE_ATTACK_VICTIM, PROC_TARGET_SELF, NULL);
			}
			else
			{
				// Proc shit
	...
	case 31878:
		{
			if(apply)
			{
				...
				// Crow: Since we absorb at parry or block or dodge, we just use absorb and melee victim.
				// Cannot guarentee it will have the desired effect though...
				CreateProcTriggerSpell(m_target, m_target->GetGUID(), GetSpellId(), 31930, Chance, PROC_ON_CAST_SPELL, PROC_TARGET_SELF, NULL);
				CreateProcTriggerSpell(m_target, m_target->GetGUID(), GetSpellId(), 54180, Chance, PROC_ON_CAST_SPELL, PROC_TARGET_SELF, NULL);
			}
			else
			{
				// Proc shit
...
void Aura::SpellAuraModStealth(bool apply)
{
	...
	else
	{
		...
		if( m_target->HasDummyAura(SPELL_HASH_MASTER_OF_SUBTLETY) && m_spellProto->NameHash == SPELL_HASH_STEALTH )
...
void Aura::SpellAuraPeriodicTriggerSpell(bool apply)
{
	...
	if(apply)
	{
		...
		if( m_spellProto->SpellFamilyFlags )
		{
			SM_FIValue( m_caster->SM[SMT_TIME][0], (int32*)&amplitude, m_spellProto->SpellFamilyFlags );
			SM_PIValue( m_caster->SM[SMT_TIME][1], (int32*)&amplitude, m_spellProto->SpellFamilyFlags );
		}
...
void Aura::SpellAuraProcTriggerSpell(bool apply)
{
	if(apply)
	{
		uint32 spellid = m_spellProto->EffectTriggerSpell[mod->i];
		if(spellid == 0)
			return;
		...
		CreateProcTriggerSpell(m_target, m_casterGuid, m_spellProto->Id, spellid, procchance, m_spellProto->procFlags, m_spellProto->procflags2, NULL, m_spellProto->procCharges, wdType);
	}
	else
	{
		for(std::list<struct ProcTriggerSpell>::iterator itr = m_target->m_procSpells.begin();itr != m_target->m_procSpells.end();itr++)
...
//		SM_FIValue(m_target->SM_FMISC_EFFECT, &amt, m_spellProto->SpellFamilyFlags);
		if(apply)
		{
			if(amt<0)
				SetNegative();
...
void Aura::EventPeriodicLeech(uint32 amount, SpellEntry* sp)
{
	...
	if(sp)
	{
		...
		SM_FFValue(m_caster->SM[SMT_MULTIPLE_VALUE][0], &coef, sp->SpellFamilyFlags);
		SM_PFValue(m_caster->SM[SMT_MULTIPLE_VALUE][1], &coef, sp->SpellFamilyFlags);
...
void Aura::EventPeriodicManaLeech(uint32 amount)
{
	...
	SM_FFValue(m_caster->SM[SMT_MULTIPLE_VALUE][0], &coef, m_spellProto->SpellFamilyFlags);
	SM_PFValue(m_caster->SM[SMT_MULTIPLE_VALUE][1], &coef, m_spellProto->SpellFamilyFlags);
	...
	if(apply)
	{
		...
		if( m_caster != NULL && m_caster->IsPlayer() )
		{
			...
			if(m_spellProto->SP_coef_override > 0)
			{
				...
				SM_FFValue( TO_PLAYER(m_caster)->SM[SMT_SP_BONUS][0], &coefmod0, m_spellProto->SpellFamilyFlags );
				SM_FFValue( TO_PLAYER(m_caster)->SM[SMT_SP_BONUS][1], &coefmod0, m_spellProto->SpellFamilyFlags );
...
void Aura::SpellAuraModDamageTakenPctPerCaster(bool apply)
{
	...
	if(apply)
	{
		m_target->DamageTakenPctModPerCaster.insert(
			make_pair(m_casterGuid, make_pair(m_spellProto->SpellFamilyFlags, mod->m_amount)));
	} 
	else
	{
		...
		{
			if(it->second.first == m_spellProto->SpellFamilyFlags)
			{
		...
		if(m_target->GetTypeId() == TYPEID_UNIT && TO_CREATURE(m_target)->GetCreatureInfo())
		{
			if(TO_CREATURE(m_target)->GetCreatureInfo()->Type != CRITTER)
			{
				if(m_target->isDead())
				{
					...
					uint32 itemid = m_spellProto->EffectSpellGroupRelation[mod->i];
...
void Aura::SpellAuraAddPctMod( bool apply )
{
	...
	int32 val = apply ? mod->m_amount : -mod->m_amount;
	if( m_spellProto->SpellFamilyFlags.Empty() )
	{
		if(sLog.IsOutDevelopement())
	...
	SendModifierLog(&m_target->SM[modifier][1], val, m_spellProto->SpellFamilyFlags, modifier, true);
}


void Aura::SendModifierLog( int32** m, int32 v, ClassFamilyMask mask, uint8 type, bool pct )
{
	...
	if( *m == 0 )
	{
		*m = new int32[SPELL_GROUPS];
		...
		{
			uint64 _mask = uint64(1) << bit;
			if(mask.IsFitToFamilyMask(_mask))
			{
				(*m)[bit] = v;
...
void Aura::SendDummyModifierLog( std::map< SpellEntry*, uint32 >* m, SpellEntry* spellInfo, uint32 i, bool apply, bool pct )
{
	...
	{
		uint64 _mask = uint64(1) << bit;
		if(spellInfo->SpellFamilyFlags.IsFitToFamilyMask(_mask))
		{
			data.group = bit;
			data.type = type;
			data.v = v;
...
void Aura::SpellAuraIncreaseHealingByAttribute(bool apply)
{
	...
	if(apply)
	{
		...
		val = mod->m_amount;
		SM_FIValue(m_caster->SM[SMT_SECOND_EFFECT_BONUS][0], &val, m_spellProto->SpellFamilyFlags);
...
void Aura::SpellAuraAddFlatModifier(bool apply)
{
	...
	sLog.outSpellDebug("Known spell modifier %u in spell %u", modifier, GetSpellId());
	SendModifierLog(&m_target->SM[modifier][0],val, m_spellProto->SpellFamilyFlags, modifier);
}
...
void Aura::SpellAuraProcTriggerWithValue(bool apply)
{
	if(apply)
	{
		...
		CreateProcTriggerSpell(m_target, m_casterGuid, m_spellProto->Id, spellid, procchance, m_spellProto->procFlags, m_spellProto->procflags2, NULL, m_spellProto->procCharges, 0, mod->m_amount );
	}
	else
	{
		for(std::list<struct ProcTriggerSpell>::iterator itr = m_target->m_procSpells.begin();itr != m_target->m_procSpells.end();itr++)
...
uint32 Aura::GetMaxProcCharges(Unit* caster)
{
	...
	if(caster)
	{
		SM_FIValue(caster->SM[SMT_CHARGES][0], (int32*)&charges, m_spellProto->SpellFamilyFlags);
		SM_PIValue(caster->SM[SMT_CHARGES][1], (int32*)&charges, m_spellProto->SpellFamilyFlags);
	}
	return charges;
}
...
uint32 procFlags, uint32 procFlags2, ClassFamilyMask *mask, int32 procCharges, uint32 wdtype, int32 procValue)
{
	if(!m_caster)
		return;
