...
WorldPacket* WorldSession::BuildQuestQueryResponse(Quest *qst)
{
	...
	*data << uint32(qst->qst_max_level);													// Quest level
	...
	*data << qst->qst_endtext;																// Subdescription
	...
	else
	{
		...
		for(uint32 i = 0; i < 4; i++)
		{
			...
			*data << uint32(qst->objectives->required_mobcount[i]);							// Kill mob count [i]
...
void QuestLogEntry::UpdatePlayerFields()
{
	...
	m_plr->SetUInt32Value(GetBaseField(m_slot), m_quest->id);

	uint32 field1 = 0, field0 = 0x04;
	uint32 i = 0;
	...
	m_plr->SetUInt32Value(GetBaseField(m_slot) + 1, field0);
	m_plr->SetUInt32Value(GetBaseField(m_slot) + 2, field1);
	m_plr->SetUInt32Value(GetBaseField(m_slot) + 3, ( m_time_left ? static_cast<uint32>(time(NULL) + (m_time_left/1000)) : 0 ));
