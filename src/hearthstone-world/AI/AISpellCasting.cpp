...
bool AIInterface::CanCastFuckingSpell(Unit* Target, AI_Spell* toCast, uint32 currentTime)
{
	...
	if(toCast->info->powerType == POWER_TYPE_MANA)
	{
		...
		//apply modifiers
		if( toCast->info->SpellFamilyFlags )
		{
			SM_FIValue(m_Unit->SM[SMT_COST][0], &cost, toCast->info->SpellFamilyFlags);
			SM_PIValue(m_Unit->SM[SMT_COST][1], &cost, toCast->info->SpellFamilyFlags);
		}
