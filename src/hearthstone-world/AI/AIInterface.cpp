...
bool AIInterface::HealReaction(Unit* caster, Unit* victim, uint32 amount, SpellEntry * sp)
{
	...
	if (sp != NULL && sp->SpellFamilyFlags)
	{
		SM_FIValue(caster->SM[SMT_THREAT_REDUCED][0],(int32*)&amount,sp->SpellFamilyFlags);
		SM_PIValue(caster->SM[SMT_THREAT_REDUCED][1],(int32*)&amount,sp->SpellFamilyFlags);
	}
...
uint32 AIInterface::GetWeaponEmoteType(bool ranged)
{
	...
	if(m_Unit != NULL && m_Unit->IsCreature())
	{
		...
		if(ctr->GetProto())
		{
			ItemPrototype* ItemE = NULL;
			if(!ranged)
			{
				uint32 weaponids[2] = { ctr->GetProto()->Item1, ctr->GetProto()->Item2 };
				if(weaponids[0])
				{
					ItemE = ItemPrototypeStorage.LookupEntry(weaponids[0]);
					if(ItemE != NULL)
					{
				...
				if(weaponids[1])
				{
					ItemE = ItemPrototypeStorage.LookupEntry(weaponids[1]);
					if(ItemE != NULL)
					{
						switch(ItemE->InventoryType)
						{
			...
			else
			{
				...
				{
					ItemE = ItemPrototypeStorage.LookupEntry(ctr->GetProto()->Item3);
